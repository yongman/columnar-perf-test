...
- Query-level improvement vs the old columnar run ranges from `1.01x` (`08_or_lookup`) to `7.38x` (`12_late_materialization_wide_topn`), with average `4.68x`.
- Relative to DeltaMerge, the new columnar run is now between `1.06x` and `10.58x` slower, with average `2.53x` and median `1.70x`.
...
| `08_or_lookup` | yes | 2.419 | 2.390 | 0.226 | 1.01x | 10.58x |
...
- The clear outlier is still `08_or_lookup`: after the retry it moved only to `2.390s`, still essentially unchanged from `2.419s`. That strongly suggests this case is not limited by raw region-level scan concurrency.
...
| `08_or_lookup` | `Sort` | 2380.0 | 2350.0 | 133.1 | `TableRangeScan` | 2380.0 | 2350.0 | 130.4 | 1 | 1 | 72 |
